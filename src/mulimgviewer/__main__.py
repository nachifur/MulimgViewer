...
class GuiManager:
    def __init__(self, UpdateUI, get_type, video_mode=False):
    ...
    def CreateFrame(self, type_):
        ...
        if type_ == 0:
            ...
            return PathSelectFrame(None, self.UpdateUI, self.get_type ,self.video_mode)
        else:
...
class MainAPP(wx.App):
    def OnInit(self):
        self.manager = GuiManager(self.UpdateUI, self.get_type, video_mode=False)
    ...
    def UpdateUI(self, type, input_path=None, parallel_to_sequential=False, video_mode=False):
        """
        type: 1=PathSelectFrame, 0=MulimgViewer, -1=全部关闭
        input_path: list[str] 或 None
        parallel_to_sequential: 保持原参数
        video_mode: 进入选择页时是否启用“视频单选”模式
        """
        self.type = type
        self.thread = 4
        if video_mode is None:
            effective_video_mode = getattr(self.manager, "video_mode", False)
        else:
